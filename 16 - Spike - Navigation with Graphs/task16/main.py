...
class BoxWorldWindow(pyglet.window.Window):
    ...
    def on_draw(self):
        ...
        if self.world.agent:
            print("pos ===>",self.world.agent.pos)
            self.world.agent.update()
            position = self.world.agent.current_pos()
            egi.blue_pen()
            egi.set_stroke(3)
            egi.circle(position,20,True)
            egi.cross(position, 5)
...
if __name__ == '__main__':
    ...
    #window = BoxWorldWindow("map2.txt")
    #window = BoxWorldWindow("map3.txt")
    window = BoxWorldWindow("map4.txt")
    pyglet.app.run()
