...
min_edge_cost = 12.0 # must be min value for heuristic cost to work
...
#            # DOWN RIGHT (i - nx + 1)
    ...
    def set_start(self, idx):
        ...
        self.start.marker = 'S'
        self.agent = Navigator(self.start._vc.x, self.start._vc.y, self.boxes)
    ...
    def plan_path(self, search, limit):
        ...
        if not self.path.path and len(self.path.open)>0:
            #self.agent.path = [random.choice(self.path.open)]
            self.agent.path = [self.path.open[-1]]
   
        elif not self.agent.path:
            self.agent.path = self.path.path
            self.agent.update()
