...
class Agent(object):
    ...
    def __init__(self, world=None, scale=30.0, mass=1.0, mode=''):
    ...
    def calculate(self,delta):
        ...
        if mode == 'seek':
            ...
            force = Vector2D()
        elif mode == 'soldier_on_patrol':
            force = self.soldier_on_patrol()
        else:
            force = Vector2D()
        self.force = force
        return force
    ...
    def render(self, color=None):
        ...
        if self.mode == 'soldier_on_patrol':
            self.path.draw_patrol_path()
            #self.path.render()
            

            self.follow_path()
            if self.target:
                self.target.pos = Vector2D(200,200)
                self.target.color = "AQUA"
                close_enough = 250
                distance_from_enemy = (self.target.pos - self.pos).length()
                if(distance_from_enemy<=close_enough):
                    self.shoot_target('rifle')
                    print("Spotted and Shooted the enemy")
                    self.follow_path()
                else:
                    print("The enemy is too far")
                    self.follow_path()
    ...
    def shoot_target(self,weapon):
        if (self.mode == 'marksmanship' or self.mode =='soldier_on_patrol') and weapon == 'rifle':
